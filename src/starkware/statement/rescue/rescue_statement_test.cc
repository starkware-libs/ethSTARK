...
TEST(RescueStatement, BasicTest) {
  ...
  EXPECT_NE(dynamic_cast<const RescueAir*>(&statement.GetAir()), nullptr);

  // Test GetInitialHashChainSeed by comparing the result to a precalculated value. The seed is
  // generated by serializing the public input to a vector of bytes. First the output, and then the
  // chain_length. Note that you will not be able to spot the output elements because they are in
  // Montgomery form. However, the chain length is easy to spot.
  // The following also tests that the public_input was parsed correctly by RescueStatement
  // constructor.
  std::vector<unsigned char> claimed_seed{
      0x10, 0x6d, 0x8d, 0x1e, 0x96, 0xa2, 0xd0, 0x34, 0x07, 0x70, 0x8a, 0xe7, 0xa1, 0x0e,
      0xcf, 0x23, 0x1e, 0x50, 0xd8, 0xc9, 0x7a, 0x34, 0xf9, 0x82, 0x09, 0x21, 0xf5, 0x25,
      0xd1, 0x1a, 0x4a, 0xa4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03};
