...
class Channel {
 public:
  virtual ExtensionFieldElement GetRandomFieldElementFromVerifier(
      const std::string& annotation) = 0;

  virtual uint64_t GetRandomNumberFromVerifier(
      uint64_t upper_bound, const std::string& annotation) = 0;

  /*
    Changes the channel seed to a "safer" seed.
    ...
    prover will have to perform 2^security_bits operations for each attempt to rig the fetched
    randomness.
  ...
  */
  ...
  /*
    ...
    generates, match the annotations in this vector. Usually, this vector contains the annotations
    created by the prover channel.
  */
  ...
  /*
    ...
    doesn't let the verifier send randomness after it is called.
  */
 ...
 private:
  ...
  /*
    Calls this function every time that the annotation scope is updated to recalculate the prefix to
