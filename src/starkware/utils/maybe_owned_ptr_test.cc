...
/*
  Counts the number of its own living instances in order to make sure it is constructed and
  destructed properly.
*/
class DummyObj {
 public:
...
TEST_F(MaybeOwnedPtrTest, UseOwned) {
  ...
  EXPECT_EQ(1, DummyObj::n_instances);
  {
    MaybeOwnedPtr<DummyObj> maybe_owned_ptr = UseOwned(&obj);
    EXPECT_EQ(1, DummyObj::n_instances);
    // Exiting the scope will free maybe_owned_ptr, but the inner object should still be alive.
  }
  EXPECT_EQ(1, DummyObj::n_instances);
  EXPECT_EQ(1, DummyObj::n_total_instances_created);
  EXPECT_EQ(0, DummyObj::n_moves);
...
TEST_F(MaybeOwnedPtrTest, UseOwnedMaybeOwned) {
  ...
  MaybeOwnedPtr<DummyObj> maybe_owned1 = UseOwned(&obj);
  {
    ...
    EXPECT_EQ(1, DummyObj::n_instances);
    // Exiting the scope will free maybe_owned2, but the inner object should still be alive.
  }
  EXPECT_EQ(1, DummyObj::n_instances);
  EXPECT_EQ(1, DummyObj::n_total_instances_created);
  EXPECT_EQ(0, DummyObj::n_moves);
...
TEST_F(MaybeOwnedPtrTest, UseOwnedUniquePtr) {
  ...
  EXPECT_EQ(1, DummyObj::n_instances);
  {
    ...
    EXPECT_EQ(1, DummyObj::n_instances);
    // Exiting the scope will free maybe_owned, but the inner object should still be alive.
  }
  EXPECT_EQ(1, DummyObj::n_instances);
  EXPECT_EQ(1, DummyObj::n_total_instances_created);
  EXPECT_EQ(0, DummyObj::n_moves);
...
TEST_F(MaybeOwnedPtrTest, PointerChanges) {
  ...
  EXPECT_EQ(1, DummyObj::n_instances);
  {
    ...
    // The following line should free the inner object.
