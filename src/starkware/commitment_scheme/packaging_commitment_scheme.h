...
/*
  ...
  (correspondingly) to enable creation of merkle_commitment_scheme_ after creating the packer_. This
  is needed because packer_ calculates number of elements in packages which is needed to create
  merkle_commitment_scheme_.
*/
using MerkleCommitmentSchemeProverFactory =
...
class PackagingCommitmentSchemeProver : public CommitmentSchemeProver {
 public:
  ...
  /*
    ...
    AddSegmentForCommitment of merkle_commitment_scheme_ with the result.
  */
  void AddSegmentForCommitment(
      gsl::span<const std::byte> segment_data, size_t segment_index) override;

  /*
    Commit to data by calling commit of merkle_commitment_scheme_.
  */
  void Commit() override;

  /*
    Starts the decommitment phase by calling merkle_commitment_scheme_ with relevant queries for
 ...
 private:
  ...
  std::unique_ptr<CommitmentSchemeProver> merkle_commitment_scheme_;
  ...
  */
  ...
  /*
    Number of elements needed for merkle_commitment_scheme to compute decommit. Initialzed with the
    correct value in StartDecommitmentPhase.
  */
  size_t n_missing_elements_for_merkle_layer_ = 0;
};
...
class PackagingCommitmentSchemeVerifier : public CommitmentSchemeVerifier {
 public:
  ...
  /*
    Call ReadCommitment of merkle_commitment_scheme_.
  */
 ...
 private:
  ...
  const PackerHasher packer_;
  std::unique_ptr<CommitmentSchemeVerifier> merkle_commitment_scheme_;
};
