/*
  Verifies a ziggy. See ziggy_prover_main.cc for more details.
*/
...
int main(int argc, char** argv) {
  ...
  ZiggyStatement statement(verifier_input.public_input, std::nullopt);

  bool result = starkware::VerifierMainHelper(
      &statement, verifier_input.proof, verifier_input.parameters, FLAGS_annotation_file);

  if (result) {
    LOG(INFO) << "Ziggy signature verified successfully.";
  } else {
    LOG(ERROR) << "Invalid Ziggy signature.";
  }
